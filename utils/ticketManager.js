  ...
  embed.addFields(
    ...
    },
    {
      ...
      value: ticketInfo.onHold
        ? `⏸️ On Hold (until wipe ends) · ${ticketInfo.claimedBy ? `Claimed by <@${ticketInfo.claimedBy}>` : 'Waiting for staff'}`
...
async function holdTicket(interaction) {
  ...
  if (!ticketInfo) {
    return interaction.reply({ content: '❌ This channel is not a registered ticket. Use /ticket-adopt first.', flags: MessageFlags.Ephemeral });
  }

  ticketInfo.onHold = !ticketInfo.onHold;
  saveGuildData(guild.id, data);

  const heldNow = ticketInfo.onHold;
  try {
    await interaction.reply({
      content: heldNow
        ? `⏸️ <@${ticketInfo.userId}>, your ticket has been placed **on hold** as **RISE** will soon begin their wipe. We'll get back to you and continue once the wipe is over — thanks for your patience!`
        : `▶️ <@${ticketInfo.userId}>, your ticket is back **active** — the wipe is over and we're ready to continue. Thanks for waiting!`,
    });
  } catch (err) {
    console.error('Hold ack failed (continuing anyway):', err.message);
  }
  ...
  if (heldNow) {
    try {
      const opener = await guild.members.fetch(ticketInfo.userId);
      const dmEmbed = new EmbedBuilder()
        .setColor(0xfaa61a)
        .setTitle('⏸️ Your ticket is on hold')
        .setDescription(`Your ticket **#${ticketNumber(ticketInfo.ticketNumber)}** in **${guild.name}** has been put on hold during the wipe. We'll continue with you once wipe is over.`)
        .setTimestamp();
...
async function acceptTicket(interaction) {
  ...
  try {
    ...
    console.error('Accept ack failed (continuing anyway):', err.message);
  }
  ...
  if (cfg.acceptRoleId && openerId) {
    try {
      ...
      roleError = err.message || String(err);
    }
  } else if (!cfg.acceptRoleId) {
    roleError = 'No accept role configured. Run /ticket-config acceptrole.';
  }
    ...
    if (cfg.logChannelId) {
      try {
        ...
        const logEmbed = new EmbedBuilder()
          ...
          .setTimestamp();
        if (roleError) {
          logEmbed.addFields({ name: '⚠️ Role NOT assigned', value: roleError.slice(0, 1024), inline: false });
        }
        const sendOptions = { embeds: [logEmbed] };
        if (transcriptFile) sendOptions.files = [transcriptFile];
        await logChannel.send(sendOptions);
...
async function handleDeclineModal(interaction) {
  ...
  try {
    ...
    console.error('Decline ack failed (continuing anyway):', err.message);
  }
  ...
  if (ticketInfo) {
    ...
    if (cfg.declinedRoleId) {
      ...
      roleError = 'No declined role configured. Run /ticket-config declinedrole.';
    }
...
async function refreshTicketPanels(client) {
  for (const guild of client.guilds.cache.values()) {
    ...
    const active = (data.tickets && data.tickets.active) || {};
    for (const [channelId, info] of Object.entries(active)) {
      if (!info.panelMsgId) continue;
      try {
        const channel = await guild.channels.fetch(channelId).catch(() => null);
        if (!channel) continue;
        const msg = await channel.messages.fetch(info.panelMsgId).catch(() => null);
        if (!msg) continue;
        const embed = buildTicketEmbed(info, guild.name);
        const row = buildTicketButtons(!!info.claimedBy, info.onHold);
...
module.exports = { showTicketModal, handleModalSubmit, claimTicket, closeTicket, handleCloseModal, handoverTicket, setTicketPriority, adoptTicket, declineTicket, handleDeclineModal, acceptTicket, holdTicket, refreshTicketPanels };
