...
function buildBoardEmbed(data, now) {
  ...
  const rows = (data.tracker.tracked || [])
    ...
    .map((t) => ({ name: t.name, ms: liveMs(t, now, boundary), online: t.online, server: t.filterServerName, discordName: t.discordName }))
    .sort((a, b) => b.ms - a.ms)
    .slice(0, MAX_NAMES);
  ...
  const embed = new EmbedBuilder()
    .setColor(GOLD)
    .setTitle('⏱️ Wipe Hours — Live')
    .setDescription(
      `Hours played on **${serverName}** since the last wipe (<t:${Math.floor(boundary / 1000)}:F>).\nUpdates automatically every 30 minutes.`
    )
  ...
  const lines = rows.map((r, i) => {
    const safeName = String(r.name).slice(0, NAME_LIMIT);
...
async function updateBoard(client, guildId) {
  ...
  try {
    ...
    if (GONE_CODES.has(err?.code)) {
      ...
      console.error('[WipeBoard] update failed (will retry):', err?.message || err);
    }
  }
}
