...
async function searchServers(query, game = 'rust') {
  const res = await axios.get(`${BASE}/servers`, {
    params: { 'filter[game]': game, 'filter[search]': query, 'page[size]': 5, 'sort': '-players' },
    headers: headers(),
  });
...
async function getPlayerBySteamId(steamId) {
  ...
  for (let attempt = 0; attempt < 4; attempt++) {
    try {
      const res = await axios.get(`${BASE}/players`, {
        params: { 'filter[identifiers]': steamId, 'page[size]': 1 },
        headers: headers(),
      });
...
module.exports = {
  searchServers, getServer, searchPlayers, getPlayer, getPlayerById,
  getPlayerSessions, getPlayerCurrentServer, getPlayerBySteamId, getPlayerServerPlaytime,
};
