...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: 'Segoe UI', system-ui, sans-serif; background: #0e0e10; color: #dcddde; min-height: 100vh; }
    .header { background: linear-gradient(135deg, #1a1a1e 0%, #111113 100%); border-bottom: 2px solid #D4AF37; padding: 24px 32px; display: flex; align-items: center; gap: 16px; }
    .header-icon { width: 48px; height: 48px; background: #D4AF37; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 22px; flex-shrink: 0; }
    .header-info h1 { font-size: 1.4rem; color: #D4AF37; font-weight: 700; letter-spacing: 0.05em; }
    .header-info p { font-size: 0.85rem; color: #9b9b9b; margin-top: 3px; }
    .meta-bar { background: #161618; border-bottom: 1px solid #2a2a2e; padding: 12px 32px; display: flex; flex-wrap: wrap; gap: 24px; }
    ...
    .messages { max-width: 900px; margin: 24px auto; padding: 0 16px 48px; display: flex; flex-direction: column; gap: 2px; }
    .message { display: flex; gap: 14px; padding: 8px 12px; border-radius: 6px; transition: background 0.1s; }
    ...
    .badge { display: inline-block; background: #5865f2; color: #fff; font-size: 0.62rem; font-weight: 700; padding: 1px 5px; border-radius: 3px; letter-spacing: 0.04em; vertical-align: middle; }
    ...
    .embed { border-left: 4px solid #D4AF37; background: #1e1e22; border-radius: 0 4px 4px 0; padding: 10px 14px; margin-top: 6px; max-width: 500px; }
    ...
    .footer { text-align: center; padding: 24px; font-size: 0.78rem; color: #4a4a4e; border-top: 1px solid #1a1a1e; }
...
<body>
  ...
  <div class="meta-bar">
    <div class="meta-item"><span class="meta-label">Opened By</span><span class="meta-value">${escapeHtml(ticketInfo.openerTag)}</span></div>
    <div class="meta-item"><span class="meta-label">Category</span><span class="meta-value">${escapeHtml(ticketInfo.category || 'General')}</span></div>
    <div class="meta-item"><span class="meta-label">Opened At</span><span class="meta-value">${formatTimestamp(ticketInfo.openedAt)}</span></div>
    <div class="meta-item"><span class="meta-label">Closed At</span><span class="meta-value">${formatTimestamp(new Date())}</span></div>
    ${ticketInfo.claimedByTag ? `<div class="meta-item"><span class="meta-label">Claimed By</span><span class="meta-value">${escapeHtml(ticketInfo.claimedByTag)}</span></div>` : ''}
  </div>
  <div class="messages">
