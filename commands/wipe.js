...
module.exports = {
  data: (() => {
    const cmd = new SlashCommandBuilder()
      ...
      .addStringOption((opt) =>
        opt.setName('title').setDescription('Title, e.g. "FEBRUARY FORCE WIPE - ATLAS US MONTHLY"').setRequired(true)
      )
      .addStringOption((opt) =>
        opt.setName('description').setDescription('Summary text (markdown supported, use \\n for new lines)').setRequired(true)
      )
      .addStringOption((opt) =>
        opt.setName('stats').setDescription('Optional stat lines, e.g. "Rockets: 12,000\\nWinnings: $2,000"').setRequired(false)
      );
  ...
  async execute(interaction) {
    ...
    for (let i = 1; i < images.length; i++) {
      embedsToSend.push(new EmbedBuilder().setColor(GOLD).setImage(images[i].url));
    }
