...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub
        .setName('staffrole')
        .setDescription('Set the staff role that can see and manage tickets')
        .addRoleOption((opt) =>
    ...
    .addSubcommand((sub) =>
      sub
        .setName('logchannel')
        .setDescription('Set the channel where transcripts are sent on close')
    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addChannelOption((opt) =>
          opt
            .setName('category')
            .setDescription('Discord channel category')
    ...
    .addSubcommand((sub) =>
      sub
        .setName('addquestion')
        .setDescription('Add a question to the ticket form (max 5 total)')
        .addStringOption((opt) =>
          opt.setName('label').setDescription('The question text').setRequired(true)
        )
        .addStringOption((opt) =>
          opt.setName('placeholder').setDescription('Placeholder hint text shown inside the box').setRequired(false)
        )
        .addBooleanOption((opt) =>
          opt.setName('paragraph').setDescription('Use a large text box? (default: false)').setRequired(false)
        )
    )
    .addSubcommand((sub) =>
    ...
    .addSubcommand((sub) =>
      sub
        .setName('acceptrole')
        .setDescription('Set the role given to users when their ticket is accepted')
        .addRoleOption((opt) =>
          opt.setName('role').setDescription('Accept role (e.g. Rise member role)').setRequired(true)
        )
    )
    .addSubcommand((sub) =>
      sub
        .setName('declinedrole')
        .setDescription('Set the role given to users when their ticket is declined (blocks future tickets)')
        .addRoleOption((opt) =>
          opt.setName('role').setDescription('Declined role').setRequired(true)
        )
    )
    .addSubcommand((sub) =>
  ...
  async execute(interaction) {
    ...
    if (sub === 'clearquestions') {
      ...
      saveGuildData(guildId, data);
      return interaction.reply({
        content: '✅ All custom questions cleared. The form will use the default question.',
        flags: MessageFlags.Ephemeral,
      });
    }
    ...
    if (sub === 'acceptrole') {
      ...
      saveGuildData(guildId, data);
      return interaction.reply({
        content: `✅ Accept role set to ${role}. Users will receive this role when their ticket is accepted.`,
        flags: MessageFlags.Ephemeral,
      });
    }
    ...
    if (sub === 'declinedrole') {
      ...
      saveGuildData(guildId, data);
      return interaction.reply({
        content: `✅ Declined role set to ${role}. Users with this role will be blocked from opening tickets.`,
        flags: MessageFlags.Ephemeral,
      });
    }
