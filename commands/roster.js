...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub
        .setName("setup")
        .setDescription("Post a new roster embed in this channel")
        .addStringOption((opt) =>
          opt
            .setName("title")
            .setDescription(
              'Team/clan name shown in the roster title (e.g. "ROI TEAM")',
            )
            .setRequired(true),
        ),
    )
    .addSubcommand((sub) =>
      sub
        .setName("add")
        .setDescription("Add a member to the roster")
        .addUserOption((opt) =>
          opt
            .setName("user")
        ...
        .addStringOption((opt) =>
          opt
            .setName("role")
            .setDescription(
              "Their role/position (e.g. CALLER, BUILDER, SUPPORT)",
            )
            .setRequired(true),
        )
        .addStringOption((opt) =>
          opt
            .setName("emoji")
            .setDescription("Emoji shown before their name (e.g. a flag 🇬🇧)")
            .setRequired(false),
        ),
    )
    .addSubcommand((sub) =>
  ...
  async execute(interaction) {
    ...
    if (sub === "add") {
      ...
      data.roster.members = data.roster.members.filter(
        (m) => m.userId !== user.id,
      );
