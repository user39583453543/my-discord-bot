...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((opt) =>
      opt.setName('title').setDescription('Panel title (default: Open a Ticket)').setRequired(false)
    )
    .addStringOption((opt) =>
      opt.setName('description').setDescription('Panel description (use \\n for new lines)').setRequired(false)
    )
    .addStringOption((opt) =>
      opt.setName('categories').setDescription('Comma-separated category names, e.g. "Support,Report,Appeal" (max 5)').setRequired(false)
    )
    .addStringOption((opt) =>
      opt.setName('banner').setDescription('Image URL to show as the banner on the panel').setRequired(false)
    ),
