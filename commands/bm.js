...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub
        .setName('steamid')
        .setDescription('Look up a player by Steam ID and show their playtime on the tracked server')
        .addStringOption((opt) =>
          opt.setName('steam-id').setDescription('Steam64 ID').setRequired(true)
        )
        .addStringOption((opt) =>
          opt.setName('server-id').setDescription('BattleMetrics server ID (default: Rustinity EU 2x Large)').setRequired(false)
        )
    ),
  ...
  async execute(interaction) {
    ...
    if (sub === 'steamid') {
      ...
      const serverId = interaction.options.getString('server-id') || '12747928';
      ...
      if (!player) return interaction.editReply('❌ No player found for that Steam ID.');

      const bmId = player.id;
      const playerName = player.attributes.name;

      // Fetch sessions filtered to the specific server
      let sessions;
      try {
        const axios = require('axios');
        const res = await axios.get(`https://api.battlemetrics.com/players/${bmId}/relationships/sessions`, {
          params: {
            'filter[servers]': serverId,
            'page[size]': 50,
          },
          headers: process.env.BATTLEMETRICS_TOKEN ? { Authorization: `Bearer ${process.env.BATTLEMETRICS_TOKEN}` } : {},
        });
        sessions = res.data.data;
      } catch {
        return interaction.editReply('❌ Failed to fetch sessions.');
      }

      if (!sessions.length) {
        return interaction.editReply(`❌ **${playerName}** has no recorded sessions on that server.`);
      }
      ...
      const lastSeen = player.attributes.updatedAt
        ? `<t:${Math.floor(new Date(player.attributes.updatedAt).getTime() / 1000)}:R>`
        : 'Unknown';

      const embed = new EmbedBuilder()
        .setColor(GOLD)
        .setTitle(`👤 ${playerName} — Playtime (last 10 days)`)
        .addFields(
          { name: '⏱️ Total', value: `${totalHrs} hrs`, inline: true },
          { name: '📅 Daily Average', value: `${avgHrs} hrs/day`, inline: true },
          { name: '\u200b', value: '\u200b', inline: true },
          { name: 'Top Days', value: topDays || 'No recent sessions', inline: false },
          { name: 'Last Seen', value: lastSeen, inline: true },
          { name: 'BM Profile', value: `[View](https://www.battlemetrics.com/players/${bmId})`, inline: true },
        )
        .setTimestamp();
