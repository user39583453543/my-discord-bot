...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub.setName('server').setDescription('Search for a Rust server')
        .addStringOption((opt) => opt.setName('search').setDescription('Server name to search').setRequired(true))
    )
    .addSubcommand((sub) =>
      sub.setName('player').setDescription('Look up a player by name')
        .addStringOption((opt) => opt.setName('name').setDescription('Player name').setRequired(true))
    )
    .addSubcommand((sub) =>
      sub.setName('track').setDescription('Track a player and get alerts when they go online/offline')
        .addStringOption((opt) => opt.setName('player-id').setDescription('BattleMetrics player ID').setRequired(true))
        .addChannelOption((opt) =>
          opt.setName('channel').setDescription('Channel to send alerts to')
            .addChannelTypes(ChannelType.GuildText).setRequired(true)
        )
        .addStringOption((opt) => opt.setName('server-id').setDescription('Only alert when the player is on this BattleMetrics server ID').setRequired(false))
    )
    .addSubcommand((sub) =>
      sub.setName('untrack').setDescription('Stop tracking a player')
        .addStringOption((opt) => opt.setName('player-id').setDescription('BattleMetrics player ID').setRequired(true))
    )
    .addSubcommand((sub) =>
      sub.setName('bulktrack').setDescription('Track up to 50 players at once by pasting Steam IDs or BattleMetrics links')
        .addChannelOption((opt) =>
          opt.setName('channel').setDescription('Channel to send alerts to')
            .addChannelTypes(ChannelType.GuildText).setRequired(true)
        )
        .addStringOption((opt) => opt.setName('server-id').setDescription('Only alert when players join this BattleMetrics server ID').setRequired(false))
    )
    .addSubcommand((sub) =>
      sub.setName('hours').setDescription('Show how many hours a player has on each server by Steam ID')
        .addStringOption((opt) => opt.setName('steam-id').setDescription('Steam ID (steamID64)').setRequired(true))
        .addStringOption((opt) => opt.setName('server').setDescription('Filter to a specific server (name or BM ID)').setRequired(false))
    )
    .addSubcommand((sub) =>
  ...
  async execute(interaction) {
    ...
    if (sub === 'server') {
      ...
      const embed = new EmbedBuilder().setColor(GOLD).setTitle(`🔍 Server Search: "${query}"`).setTimestamp();
    ...
    if (sub === 'player') {
      ...
      try {
        ...
        if (err.response?.status === 401) return interaction.editReply('❌ A BattleMetrics API token is required for player lookups. Set `BATTLEMETRICS_TOKEN` in your secrets.');
        return interaction.editReply('❌ Failed to reach BattleMetrics API.');
      }
      if (!players.length) return interaction.editReply('No players found.');
      const embed = new EmbedBuilder().setColor(GOLD).setTitle(`👤 Player Search: "${name}"`).setTimestamp();
    ...
    if (sub === 'track') {
      ...
      if (already) return interaction.reply({ content: '⚠️ That player is already being tracked.', flags: MessageFlags.Ephemeral });
      await interaction.deferReply({ flags: MessageFlags.Ephemeral });
      let player;
      try {
        player = await bm.getPlayer(playerId);
      } catch (err) {
        if (err.response?.status === 401) return interaction.editReply('❌ A BattleMetrics API token is required. Set `BATTLEMETRICS_TOKEN` in your secrets.');
    ...
    if (sub === 'untrack') {
      ...
      if (data.tracker.tracked.length === before) return interaction.reply({ content: '❌ That player is not being tracked.', flags: MessageFlags.Ephemeral });
      saveGuildData(guildId, data);
    ...
    if (sub === 'bulktrack') {
      ...
      const input = new TextInputBuilder()
        ...
        .setLabel('SteamID or BM link + name — 1 per line')
    ...
    if (sub === 'hours') {
      ...
      if (!/^\d{17}$/.test(steamId)) return interaction.editReply('❌ That doesn\'t look like a valid Steam ID. Use the 17-digit steamID64 (e.g. `76561198000000000`).');
      let player;
      try {
      ...
      if (!player) return interaction.editReply(`❌ No Rust player found for Steam ID \`${steamId}\`.`);
      let servers;
      try {
      ...
      await Promise.all(top.map(async (s) => {
        if (!s.name) {
          try { const sv = await bm.getServer(s.id); s.name = sv.attributes.name; }
          catch { s.name = `Server ${s.id}`; }
        }
      }));
      ...
      const embed = new EmbedBuilder()
        ...
        .setTimestamp();
      if (!top.length) {
        embed.setDescription(serverFilter ? `No matching server found for **${player.attributes.name}** (filter: "${serverFilter}").` : 'No server playtime found for this player.');
      } else {
    ...
    if (sub === 'watchlist') {
      ...
      if (!list.length) return interaction.reply({ content: 'No players are being tracked. Use `/bm track` to add one.', flags: MessageFlags.Ephemeral });
      const embed = new EmbedBuilder().setColor(GOLD).setTitle('📋 Tracked Players').setTimestamp();
      for (const t of list) {
